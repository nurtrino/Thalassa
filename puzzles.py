...
Five interactive/generated kinds (client never receives fields in "secret"):
  ...
  · tetromino  Talos-Principle-style sigil fill: tile a 4×5 region exactly
               with the given tetromino pieces. Generated by actually tiling
               the region first, so always solvable.
...
TIME_LIMITS = {"riddle": 60, "tetromino": 100, "nonogram": 30,
               "simon": 50, "anagram": 60, "ravens": 75}
...
def check_tetromino(data: dict, assignment) -> bool:
    ...
    for idx, cells in groups.items():
        if len(cells) != 4 or _normalize(cells) not in ROTATIONS[pieces[idx]]:
            return False
    return True
